...
import { Home, MessageSquare, ThumbsUp, Search, Plus, Lock, Map as MapIcon, BookOpen } from 'lucide-vue-next'
...
const sidebarItems = computed(() => [
  ...
  { icon: markRaw(MapIcon), label: t('nav.learning_path') || 'Ruta de Aprendizaje', href: '/estudiante/ruta' },
  { icon: markRaw(BookOpen), label: t('postulante.digital_log') || 'Bitácora', href: '/estudiante/bitacora' },
  { icon: markRaw(MessageSquare), label: 'Foro UTP+', href: '/estudiante/foro' },
]);
    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="space-y-6">
        ...
        <Card class="border-gray-200 shadow-sm bg-white overflow-hidden">
          <div class="h-2 bg-gradient-to-r from-[#B50E30] to-[#8F0B26]"></div>
          <CardContent class="p-5">
            <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
              <div class="relative w-full md:max-w-md">
                <Search class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                <Input placeholder="Buscar discusiones, preguntas o temas..." class="pl-9 bg-gray-50 border-gray-200 focus-visible:ring-[#B50E30]" />
              </div>
              <Button @click="() => requireAuth() && (isWriting = true)" class="w-full md:w-auto bg-[#B50E30] hover:bg-[#8F0B26] text-white shadow-md">
            ...
            <div class="flex flex-wrap gap-2 mt-5">
              <button
                ...
                :class="selectedCategory === cat ? 'bg-[#B50E30] text-white border-[#B50E30] shadow-sm' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300 hover:bg-gray-50'"
              >
                {{ cat }}
              </button>
            </div>
        ...
        <Card v-if="isWriting" class="border-[#B50E30]/20 shadow-md bg-red-50/30">
          <CardHeader class="pb-3 border-b border-gray-100">
            <CardTitle class="text-lg flex items-center gap-2">
              <MessageSquare class="w-5 h-5 text-[#B50E30]" />
          ...
          <CardContent class="pt-5 space-y-4">
            ...
            <div class="flex items-center justify-between pt-2">
              <div class="flex items-center gap-2">
                ...
                <select v-model="newPostCategory" class="text-sm border-gray-300 rounded-md bg-white py-1.5 px-3 focus:outline-none focus:ring-2 focus:ring-[#B50E30]">
              ...
              <div class="flex gap-2">
                ...
                <Button @click="submitPost" class="bg-[#B50E30] hover:bg-[#8F0B26]" :disabled="!newPostTitle || !newPostContent">Publicar</Button>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <div class="space-y-4">
            ...
            <CardContent class="p-5">
              <div class="flex items-start gap-4">
                <Avatar class="w-10 h-10 ring-2 ring-gray-100">
                  <AvatarFallback class="bg-[#B50E30] text-white font-bold">{{ getInitials(post.autor?.nombre, post.autor?.apellido) }}</AvatarFallback>
                </Avatar>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 mb-1 flex-wrap">
                    <span class="font-bold text-gray-900">{{ post.autor?.nombre }} {{ post.autor?.apellido }}</span>
                    <Badge variant="secondary" class="bg-red-50 text-[#B50E30] text-[10px] uppercase font-bold tracking-wider">
                  ...
                  <h3 class="text-lg font-bold text-gray-900 mt-2 mb-1 transition-colors cursor-pointer hover:text-[#B50E30]" @click="toggleComments(post.id)">
                    {{ post.titulo }}
                  </h3>
                  ...
                  <div class="flex items-center gap-4 mt-4 pt-4 border-t border-gray-100">
                    ...
                    <button @click="toggleComments(post.id)" class="flex items-center gap-1.5 text-sm font-medium text-gray-500 hover:text-[#B50E30] transition-colors">
                  ...
                  <div v-if="expandedPostId === post.id" class="mt-4 pt-4 border-t border-gray-100 space-y-4">
                    ...
                    <div class="flex gap-3 mt-4 items-start">
                      <Avatar class="w-8 h-8">
                        <AvatarFallback class="bg-[#B50E30] text-white font-bold">Tú</AvatarFallback>
                      </Avatar>
                      <div class="flex-1 space-y-2">
                        ...
                        <div class="flex justify-end">
                          <Button size="sm" @click="submitComment(post.id)" class="bg-[#B50E30] hover:bg-[#8F0B26]" :disabled="!newCommentContent[post.id]">Responder</Button>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div v-if="filteredPosts.length === 0" class="text-center py-12 bg-white rounded-xl border border-dashed border-gray-300">
            ...
            <Button @click="() => requireAuth() && (isWriting = true)" variant="outline" class="mt-4 border-[#B50E30] text-[#B50E30]">Crear publicación</Button>
          </div>
        </div>
      </div>
    </div>
