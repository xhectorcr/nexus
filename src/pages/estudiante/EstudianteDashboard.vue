...
const completedCount = computed(() => {
  ...
  return activeJourney.value.nodos.filter((n: any) => n.estado === 'COMPLETADO').length;
});
...
const fetchEstudianteData = async () => {
  ...
  try {
    ...
    if (perfilRes.data?.data) {
      ...
      statsValues.value.cursosActuales = (perfil.cursosActuales || (activeJourney.value ? 1 : 0)).toString();
      statsValues.value.promedioAcumulado = (perfil.promedioAcumulado || (completedCount.value > 0 ? (16 + Math.min(completedCount.value * 0.5, 4)).toFixed(1) : "0.0")).toString();
      statsValues.value.creditosAprobados = (perfil.creditosAprobados || (completedCount.value * 2)).toString();
      statsValues.value.asistenciaGlobal = (perfil.asistenciaGlobal || (completedCount.value > 0 ? 100 : 0)).toString() + "%";
